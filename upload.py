...
class Logger:
    ...
    def Log(text, color):
        ...
        try:
            with open(os.path.join(_DATA_DIR, "logs.txt"), "a") as f:
...
class Threads():
    ...
    def login_with_session_file(self, session_file: str, sessionid: str):
        ...
        without making any API calls — avoids datacenter IP blocks entirely.
        """
        import base64 as _b64
        self.cl.load_settings(session_file)
        s = self.cl.get_settings()

        # Restore device + user agent from saved session
        if s.get("device_settings"):
            self.cl.set_device(s["device_settings"])
        if s.get("user_agent"):
            self.cl.set_user_agent(s["user_agent"])
        if s.get("uuids"):
            self.cl.set_uuids(s["uuids"])

        # Extract user id from sessionid (first numeric segment)
        ds_user_id = sessionid.split("%3A")[0].split(":")[0]
        self.user_id = int(ds_user_id)

        # Construct Bearer token — identical to what Instagram generates
        ...
        # Set sessionid as a proper Cookie on the requests session
        self.auth_session.cookies.set("sessionid", urllib.parse.unquote(sessionid), domain=".instagram.com")
        self.auth_session.cookies.set("ds_user_id", ds_user_id, domain=".instagram.com")
...
def get_accounts() -> list[str]:
    conn = sqlite3.connect(os.path.join(_DATA_DIR, "thread", "accounts.db"), check_same_thread=False)
    cursor = conn.cursor()
...
def upload(username, password, email, email_password, proxy, thread_index: int, captions: list = [], images: str | list | None = None, comment_count: int = 20, search_query: str | None = None, spam_method: int = 0, captcha_key: str = "", cookies: str = "", serialized_settings: str = "", telegram_token: str = "", telegram_chat_id: str = "", original_spam_method: int = 0, max_time_seconds: int = 0, minimum_likes: int = 0, minimum_replies: int = 0, disable_comments: bool = False, avatar_folder: str = "", max_posts_on_post: int = 3, min_views_on_post: int = 500, post_links: list = [], set_image_on_rec: bool = True, set_image_on_warm: bool = True, search_queries: list = []):
    ...
    if cookies == "":
        if password == "SESSION_FILE":
            # accounts.txt format: username:SESSION_FILE:path/to/session.json:sessionid
            session_file = email
    ...
    _history_dir = os.path.join(_DATA_DIR, "history", f"{today.month}.{today.day}")
    ...
    if posts_counts >= comment_count:
        ...
        with open(os.path.join(_history_dir, "successful_warm.txt"), "a") as f:
